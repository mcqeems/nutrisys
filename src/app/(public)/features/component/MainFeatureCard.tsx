...
export function MainFeatureCard({
  ...
  return (
    <motion.div 
      ...
      className="group relative overflow-hidden rounded-2xl bg-card border border-border/40 transition-all duration-500 cursor-pointer hover:shadow-2xl hover:border-primary/50"
    ...
    >
      ...
      <div className="relative p-8">
        ...
        <div className="flex items-start justify-between mb-6">
          ...
          <div className="text-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0">
            →
          </div>
        </div>
        ...
        <div className="mt-6 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
          <Link href="/login">
            <button className="w-full py-3 rounded-lg font-semibold bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-300 transform hover:scale-105 active:scale-95">
              Coba {feature.title}
            </button>
          </Link>
        </div>
